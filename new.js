...
class Fruit {
    ...
    collide(other) {
        ...
        if (dist <= rsum) {
            if (this.ri === other.ri) {
                ...
                let n1 = other.pos.newDiff(this.pos).multiply(1 / dist);
                let n2 = this.pos.newDiff(other.pos).multiply(1 / dist);
                let v1 = this.pos.newDiff(this.lastPos);
                let v2 = other.pos.newDiff(other.lastPos);
                v1 = n1.newMultiply(v1.x * n1.x + v1.y * n1.y);
                v2 = n2.newMultiply(v2.x * n2.x + v2.y * n2.y);
                let c = 0.0;
                let u1 = v1.newMultiply((this.m - c * other.m) / (this.m + other.m)).add(
                    v2.newMultiply(((1 + c) * other.m) / (this.m + other.m))
                );
                let u2 = v2.newMultiply((other.m - c * this.m) / (this.m + other.m)).add(
                    v1.newMultiply(((1 + c) * this.m) / (this.m + other.m))
                );
                ...
                this.pos.diff(v1).add(u1).add(d1);
                // this.lastPos = this.pos.newDiff(u1);
                // this.pos = this.lastPos.newAdd(u1);

                other.pos.diff(v2).add(u2).add(d2);
                // other.lastPos = other.pos.newDiff(u2);
                // other.pos = other.lastPos.newAdd(u2);
    ...
    keepInside(maxX, maxY) {
        ...
        if (this.pos.y > maxY - this.r) {
            ...
            this.lastPos.y = this.pos.y + d_y * 0.2;
        }
        if (this.pos.y < this.r + Y_ZERO) {
            // this.lastPos = this.pos.copy()
            let d_y = this.pos.y - this.lastPos.y;
            this.pos.y = this.r + Y_ZERO;
            this.lastPos.y = this.pos.y + d_y * 0.6;
        }
    }
    rotate() {
...
class Scene {
    constructor() {
        ...
        this.fruits = [
            new Fruit(new Pos(100, 140), 5, 1, new Pos(85, 150)),
            new Fruit(new Pos(200, 140), 4, 1, new Pos(210, 150)),
        ];
